...
// On gère la recheerche d'utilisateurs
...
let messages = [
  ...
  {
    type: "receive",
    content: "Duis ullamcorper egestas placerat. Nunc sed sodales mi. Nullam venenatis elit non hendrerit ullamcorper"
  },
  {
    type: "receive",
    content: "yes"
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if(messages[i]['type'] == 'receive'){
      ...
      if(i > 0 && $('#message-'+j).hasClass('message-other')){
        ...
        $('#message-'+j+' .logo').removeClass('logo-color').addClass('logo-transparent');
        // On supprime les margin

      }
      // On affiche le message
      $('#messages').append('<div class="message-other" id="message-'+i+'"><div class="logo logo-color"></div><div class="message">'+messages[i]['content']+'</div></div>');
    }
    // Si le message a été envoyé par l'utilisateur
    else if(messages[i]['type'] == 'send'){
      // On affiche le message
      $('#messages').append('<div class="message message-my" id="message-'+i+'">'+messages[i]['content']+'</div>');
    }
  }
}
displayMessages();
